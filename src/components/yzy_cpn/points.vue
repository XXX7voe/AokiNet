<template>
<!-- 我的积分界面 -->
    <div id="points" class="animated fadeIn">
        <div class="head">
            <!-- 返回到我的界面 -->
            <i class="el-icon-arrow-left" @click="mySelf()"></i>
            <span>我的积分</span>
        </div>
        <div class="center">
            <span>当前积分</span>
            <i class="el-icon-question"></i>
            <span @click="pointsinstructions()">积分说明</span>
            <p>0</p>
            <span>分</span>
            <button class="withdrawalBtn">积分兑换商品</button>
        </div>
        <div class="foot">
            <p>最近30天积分记录</p>
            <img class="no_logo" src="data:image/png;base64," alt="">
            <p class="pp">最近30天无积分记录</p>
            <span>快去下单赚取大量积分吧</span>
        </div>
    </div>
</template>
<script>
export default {
    name: 'points',
    data() {
        return {
            
        };
    },
    mounted() {
        
    },
    methods: {
        // 跳转我的界面
        mySelf(){
            this.$router.push({
                name:'mySelf'
            })
        },
        // 跳转余额说明
        pointsinstructions(){
            this.$router.push({
                name:'pointsInstructions'
            })
        }
    },
};
</script>

<style  scoped>
.head{
    background-color: #3190e8;
    width: 100%;
    height: 2.2rem;
    position: relative;
}
.el-icon-arrow-left{
    font-size: 0.25rem;
    margin-top: 0.11rem;
    color: white;
}
.head span{
    font-size: 0.2rem;
    margin-left: 1.2rem;
    color: white;
}
.center{
    width: 96%;
    height: 1.65rem;
    background-color: rgb(255,255,255);
    position: relative;
    top: -1.72rem;
    left: 0.08rem;
    border-radius: 3%;
}
.center :nth-child(1){
    position: absolute;
    top: 0.1em;
    left: 0.1rem;
}
.el-icon-question{
    position: absolute;
    left: 2.5rem;
    top: 0.1rem;
    color: #3190e8;
}
.center :nth-child(3){
    position: absolute;
    top: 0.088rem;
    left: 2.7rem;
    color: #3190e8;
}
.center :nth-child(4){
    position: absolute;
    top: 0.8em;
    left: 0.15rem;
    font-size: 0.4rem;
    color: black;
}
.center :nth-child(5){
    position: absolute;
    top: 0.5rem;
    left: 0.4rem;
}
.withdrawalBtn{
    position: absolute;
    width: 90%;
    height: 0.5rem;
    outline: none;
    border:0;
    background-color: #FF6235;
    color: white;
    top: 0.95rem;
    left: 0.2rem;
}
.foot{
    top: -1.5rem;
    left: 0.1rem;
    position: relative;
}
.foot :nth-child(1){
    color: #999;
    font-size: 0.15rem;
}
.no_logo{
    width: 2.3rem;
    position: absolute;
    left: 0.5rem;
    top: 0.7rem;
}
.pp{
    font-size: 0.15rem;
    color: #666;
    position: absolute;
    left: 0.95rem;
    top: 2.3rem;
}
.foot span{
    font-size: 0.14rem;
    color: #999;
    position: absolute;
    left: 0.85rem;
    top: 2.7rem;
}
</style>